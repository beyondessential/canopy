...
pub fn routes() -> OpenApiRouter<AppState> {
	OpenApiRouter::new()
		...
		.routes(routes!(write: create_repo))
		.routes(routes!(danger: request_now))
		.routes(routes!(write: cancel_request))
		.routes(routes!(read_only: restore_window))
		.routes(routes!(danger: allow_restore))
		.routes(routes!(write: disallow_restore))
		.routes(routes!(write: request_maintenance))
		.routes(routes!(write: cancel_maintenance))
		.routes(routes!(read_only: stats))
		.routes(routes!(read_only: run_progress))
		.routes(routes!(read_only: capabilities))
		.routes(routes!(danger: set_capability))
		.routes(routes!(danger: delete))
		.routes(routes!(read_only: recovery_status))
		.routes(routes!(write: recovery_challenge))
		.routes(routes!(write: recovery_verify))
}
